        ...
        extends AbstractConfigurationAwareModule
{
    ...
    protected void setup(Binder binder)
    {
        ...
        // getCacheFactory() is only invoked when caching is requested: the ConnectorContext
        // default throws, and contexts predating the cache SPI must still construct
        // non-caching catalogs
        ...
        if (config.isCacheEnabled()) {
            ...
            BlobCache blobCache = context.getCacheFactory().createBlobCache(DATA_CACHE_REQUIREMENTS)
            ...
            blobCacheBinder.setBinding().toInstance(blobCache);
            if (isCoordinator) {
            ...
            context.getCacheFactory().createBlobCache(METADATA_CACHE_REQUIREMENTS)
                    .ifPresent(cache -> blobCacheBinder.setBinding().toInstance(cache));
        }
    }
