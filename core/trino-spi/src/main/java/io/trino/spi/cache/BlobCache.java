...
public interface BlobCache
{
     ...
     * <p>
     * Invalidation is best effort: implementations that cannot enumerate entries by prefix
     * may invalidate only some of the matching entries, or none at all. Callers must not
     * rely on it for correctness and should instead encode the content identity (such as a
     * file version) in the key, so that stale entries are never looked up; invalidation
     * only helps reclaim their space sooner.
     */
    void invalidate(CacheKey prefix);
}
